...
class Program
{
    ...
    static void MostrarEstadisticas(List<Paciente> pacientes)
    {
        // Estadísticas por prioridad
        var pacientesPorPrioridad = pacientes.GroupBy(p => p.Prioridad)
                                            .Select(g => new
                                            {
                                                Prioridad = g.Key,
                                                Cantidad = g.Count(),
                                                TiempoEsperaPromedio = g.Average(p => p.TiempoEspera.Elapsed.TotalSeconds)
                                            });

        // Uso de las máquinas de diagnóstico
        int totalDiagnosticos = pacientes.Count(p => p.RequiereDiagnostico);
        double usoPromedioMaquinas = (double)totalDiagnosticos / 2; // 2 máquinas disponibles

        Console.WriteLine("\n--- ESTADÍSTICAS FINALES ---");
        foreach (var grupo in pacientesPorPrioridad)
        {
            Console.WriteLine($"Pacientes con prioridad {grupo.Prioridad}: {grupo.Cantidad}");
            Console.WriteLine($"Tiempo promedio de espera: {grupo.TiempoEsperaPromedio:F2} segundos");
        }

        Console.WriteLine($"\nUso promedio de las máquinas de diagnóstico: {usoPromedioMaquinas:F2} pacientes por máquina");
    }
}